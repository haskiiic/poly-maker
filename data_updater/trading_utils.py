...
def approveContracts():
    ...
    # Use absolute path for erc20ABI.json
    current_dir = os.path.dirname(os.path.abspath(__file__))
    abi_path = os.path.join(current_dir, "erc20ABI.json")
    ...
    for address in [
        ...
        signed_usdc_txn = web3.eth.account.sign_transaction(
            raw_usdc_txn, private_key=os.getenv("PK")
        )
        usdc_tx_receipt = web3.eth.wait_for_transaction_receipt(
            web3.eth.send_raw_transaction(signed_usdc_txn.raw_transaction), 600
        )
        ...
        raw_ctf_approval_txn = ctf_contract.functions.setApprovalForAll(
            address, True
        ).build_transaction(
        ...
        signed_ctf_approval_tx = web3.eth.account.sign_transaction(
            raw_ctf_approval_txn, private_key=os.getenv("PK")
        )
    ...
    raw_txn_2 = usdc_contract.functions.approve(
        "0xC5d563A36AE78145C45a50134d48A1215220f80a", int(MAX_INT)
    ).build_transaction({"chainId": 137, "from": wallet.address, "nonce": nonce})
    signed_txn_2 = web3.eth.account.sign_transaction(
        raw_txn_2, private_key=os.getenv("PK")
    )
    send_txn_2 = web3.eth.send_raw_transaction(signed_txn_2.raw_transaction)

    nonce = web3.eth.get_transaction_count(wallet.address)
    raw_txn_3 = usdc_contract.functions.approve(
        "0xd91E80cF2E7be2e162c6513ceD06f1dD0dA35296", int(MAX_INT)
    ).build_transaction({"chainId": 137, "from": wallet.address, "nonce": nonce})
    signed_txn_3 = web3.eth.account.sign_transaction(
        raw_txn_3, private_key=os.getenv("PK")
    )
