...
def get_all_orders(client):
    ...
    if len(orders_df) > 0:
        orders_df["order_size"] = orders_df["original_size"].astype(
            "float"
        ) - orders_df["size_matched"].astype("float")
        orders_df = orders_df[["asset_id", "order_size", "side", "price"]]

        orders_df = orders_df.rename(
            columns={"side": "order_side", "price": "order_price"}
        )
        return orders_df
    else:
        return pd.DataFrame()
...
def combine_dfs(orders_df, positions, markets_df, selected_df):
    merged_df = orders_df.merge(
        positions, left_on=["asset_id"], right_on=["asset"], how="outer"
    )
    merged_df["asset_id"] = merged_df["asset_id"].combine_first(merged_df["asset"])
    merged_df = merged_df.drop(columns="asset", axis=1)

    merge_token1 = merged_df.merge(
        markets_df, left_on="asset_id", right_on="token1", how="inner"
    )
    merge_token1["merged_with"] = "token1"

    # Merge with token2
    merge_token2 = merged_df.merge(
        markets_df, left_on="asset_id", right_on="token2", how="inner"
    )
    ...
    combined_df["answer"] = combined_df.apply(
        lambda row: row["answer1"]
        if row["merged_with"] == "token1"
        else row["answer2"],
        axis=1,
    )
    ...
    combined_df["marketInSelected"] = combined_df["question"].isin(
        selected_df["question"]
    )
