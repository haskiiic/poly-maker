...
def check_funds_location():
    ...
    try:
        ...
        for order in orders:
            ...
            if order['side'] == 'BUY':
                amount = float(order['price']) * float(order['original_size']) # Approximation
                locked_in_orders += amount
                print(f"  - Buy Order: {order['size_matched']}/{order['original_size']} @ {order['price']} (Locked ~{amount:.2f} USDC)")
    ...
    try:
        import requests
        res = requests.get(f'https://data-api.polymarket.com/positions?user={target_address}')
        if res.status_code == 200:
            ...
            val_res = requests.get(f'https://data-api.polymarket.com/value?user={target_address}')
    ...
    total_accounted = usdc_balance + (locked_in_orders if 'locked_in_orders' in locals() else 0) + (pos_value if 'pos_value' in locals() else 0)
